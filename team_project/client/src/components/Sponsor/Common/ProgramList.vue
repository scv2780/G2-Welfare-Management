        ...
        <tbody>
          ...
          >
            ...
            <td class="priority-align-right">
              {{ numberFormat(program.goal_amount) }}원
            </td>
            <td class="priority-align-right">
              {{ numberFormat(program.current_amount) }}원
            </td>
            <td>
              <button
...
const statusClass = (statusValue) => {
  switch (statusValue) {
    case "진행전":
      return "p-gray"; // 요청(BA1)과 유사한 색상으로 매핑
    case "진행중":
      return "p-blue"; // 승인(BA2)과 유사한 색상으로 매핑
    case "완료":
      return "p-green"; // 반려(BA3)와 유사한 색상으로 매핑
    case "중단":
      return "p-red"; // 반려(BA3)와 유사한 색상으로 매핑
    default:
      return "";
  }
};
